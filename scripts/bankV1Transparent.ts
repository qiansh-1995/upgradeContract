...
async function main() {
    const OpenProxy = await ethers.getContractFactory('MyPiggyBankTransparent')
    const mytokenAddress="0xae708e26b4529C7c811fb22D6aCf03da8d868B73";
    // 部署合约, 并调用初始化方法
    const myOpenProxy = await upgrades.deployProxy(OpenProxy,[mytokenAddress], {
   ...
   // console.log(`proxyAddress: ${proxyAddress}`)
    ...
    console.log(
        `Use "npx hardhat verify  0x99fB4648B14259a3b5643f87Af7A2727b95B1EF6 --network bscTestnet  --contract "contracts/MyPiggyBankTransparent.sol:MyPiggyBankTransparent" " to verify`
      );
}
